...
wWinMain(HINSTANCE instance, HINSTANCE pre_instance, PWSTR cmd_line, int show_cmd)
{
  int exit_code = 0;
	...
	{
		HANDLE bitmap = LoadImageW(0, L"W:\\tet\\build\\spritesheet.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
		ASSERT(bitmap != 0);
  ...
  for (bool running = true;;)
  {
    for (MSG msg; PeekMessageW(&msg, 0, 0, 0, PM_REMOVE); )
    {
               ...
               msg.message == WM_KEYDOWN    && msg.wParam == VK_F11    && ((msg.wParam>>30)&1) == 0)
      {
		...
		V2S board_dim    = V2S(10, 20);
		BlitFrame(V2S_Sub(board_origin, V2S(0, 3)), V2S(board_dim.x, 1));
		BlitString(V2S_Add(board_origin, V2S(2, -2)), "LINES-000");
		...
		BlitString(V2S_Add(score_origin, V2S(1, 3)), "000000");
		BlitString(V2S_Add(score_origin, V2S(1, 5)), "SCORE");
		BlitString(V2S_Add(score_origin, V2S(1, 6)), "000000");
		...
		BlitString(V2S_Add(level_origin, V2S(1, 2)), "  05 ");

		for (umm i = 1; i < 11; ++i) BlitSprite(V2S(board_origin.x + i, board_origin.y + 20), V2S(5, 4));
